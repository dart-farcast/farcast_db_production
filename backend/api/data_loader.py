"""
FarCast DB v2 — Data Loader
All pandas I/O lives here. Called ONCE at startup, results cached in memory.
"""
import os, re
import pandas as pd
...
def discover_assays() -> dict:
    # Deprecated. No longer using CSVs.
    return {}

def load_assay_dfs(assay_paths: dict = None) -> dict:
    """Return {name: DataFrame} for every assay table in PostgreSQL."""
    dfs = {}
    print("Loading Assays from PostgreSQL...")
    try:
        from sqlalchemy import create_engine, inspect
        engine = create_engine("postgresql://farcast:farcastpassword@localhost:5433/farcast_db")
        insp = inspect(engine)
        tables = insp.get_table_names()
        for table in tables:
            if table.startswith('assay_'):
                # Extract assay name e.g. "assay_flow_cytometry" -> "Flow Cytometry"
...
def build_indexes(meta: pd.DataFrame, overlay: pd.DataFrame) -> dict:
    ...
    for _, row in overlay.iterrows():
        ...
        if a:
            arm_index.setdefault(a.lower(), set()).add(s)  # lowercase key for case-insensitive match
    ...
    # also build sets for all sample_ids (for unfiltered baseline)
    all_sids = set(meta['Sample_ID'].unique())

    # Build set for qualified sample_ids based on Final Qualification column
